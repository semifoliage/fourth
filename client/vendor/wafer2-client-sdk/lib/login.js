 ...
 */
function getWxLoginResult (opts, cb) {

    //package ecode, iv and nickname
    var code = {
        encryptedData: opts.data.detail.encryptedData,
        iv:opts.data.detail.iv,
        nickName:opts.data.detail.userInfo.nickName
    };
    wx.login({
        success (loginResult) {
            wx.getUserInfo({
                success (userResult) {
                    console.log('getwxloginResult wx.login')
...
function login (opts) {
    ...
    getWxLoginResult(opts, (err, loginResult) => {
        if (err) {
        ...
        console.log('login getwxloginresult');
...
function loginWithCode (opts) {
    opts = Object.assign({}, defaultOptions, opts)
    console.log('loginwithcode opts');
    console.log(opts);
    if (!opts.loginUrl) {
        return opts.fail(new Error('登录错误：缺少登录地址，请通过 setLoginUrl() 方法设置登录地址'))
    }
...
function requestUserId(opts){
    opts = Object.assign({}, defaultOptions, opts)
    console.log('requestUserId opts');
    console.log(opts);
    if (!opts.loginUrl) {
            return opts.fail(new Error('登录错误：缺少登录地址，请通过 setLoginUrl() 方法设置登录地址'))
        }
     ...
     // 请求服务器登录地址，获得会话信息
        wx.request({
            ...
            data: {
                name: opts.data.codeName
            },
            method: opts.method,
            success (result) {
                const data = result.data;
                console.log('requestUserId collected ')
...
module.exports = { login, setLoginUrl, loginWithCode, requestUserId }
