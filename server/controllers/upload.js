...
module.exports = async ctx => {
    ...
    //ocr
    console.log('upload await.................');
    const dataImage = await uploader(ctx.req);
    //const { data: ocrResult } = await ci.ocr(data.imgUrl, 'qcloudtest', 0);
    var imgUrl = dataImage.imgUrl;
    console.log('data.image ------');
    console.log(dataImage.name);
    
    ctx.response.type = 'JSON';
    
  ctx.state.data =dataImage;
  //responce body
    ctx.body =dataImage;//ctx.req;// query2;//queryString.parse( ctx.req);
};
